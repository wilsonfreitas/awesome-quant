...
- [Futures Clock](https://futuresclock.com/en/) - `Web` `JSON` - Free live open/closed clock and reference for 65 futures products on 16 exchanges (CME, COMEX, NYMEX, CBOT, Eurex, LME, ICE, SGX, OSE, HKEX, SHFE, DCE, CZCE, GFEX, CFFEX, INE) with DST-aware session windows, night sessions, 2026 holiday calendars and contract specs transcribed from official exchange publications, bilingual EN/ZH, plus an open CORS JSON endpoint (/data/trading-hours.json).
