...
- [YieldScope](https://yieldscope.io/api/rates.json) - `Data` - Free JSON feed of base (non-promotional) APY rates across 37 crypto venues — CeFi exchanges and liquid-staking protocols — with an A–F safety grade per venue scored on five published risk criteria, plus borrow rates. No API key, CORS enabled, refreshed daily.
