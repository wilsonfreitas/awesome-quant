...
- [dexpaprika-sdk-python](https://github.com/coinpaprika/dexpaprika-sdk-python) - `Python` - Free DEX data API client. 36 blockchains, 36M+ pools, 33M+ tokens, real-time SSE streaming, OHLCV. No API key needed.
...
- [Anomaly Replication Lab](https://factors.austinlev.dev) - Free web app replicating 18 published equity-factor anomalies from public data (Ken French, AQR), showing in-sample versus post-publication returns and Newey-West t-statistics against multiple-testing-adjusted significance thresholds. No account required.
- [Volatility Risk Premium Lab](https://vrp.austinlev.dev) - Free web app measuring the spread between option-implied and subsequent realized volatility for the S&P 500, Nasdaq-100, crude oil and gold, with a short-straddle backtest and volatility-regime-conditional lookups. No account required.
- [Portfolio Lab](https://portfolio-lab.austinlev.dev) - Free web app that decomposes a portfolio into asset-class exposures and Fama-French factor loadings, flags overlapping funds and total fees, and runs block-bootstrap Monte Carlo projections. No account required.
- [Earnings Gap Research](https://earnings-gap.austinlev.dev) - Free web app testing whether the size of a stock's earnings-day price gap predicts subsequent market-excess returns across roughly 10,000 S&P 500 events, with per-company event-study charts. No account required.
