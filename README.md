...
- [bykaranteli-mcp](https://github.com/bykarantelicom/bykaranteli-mcp) - `TypeScript` `MCP` - Read-only MCP server exposing crypto derivatives and market data through ByKaranteli's public JSON API; runs locally with a free account key or via a hosted endpoint that needs none. [Website](https://bykaranteli.com/developers)
