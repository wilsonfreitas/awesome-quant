...
- [mx-backtest](https://github.com/27dream/mx-backtest) - `Python` - Event-driven backtesting engine for China A-shares that properly handles T+1 settlement, price limit circuit breakers, trading halts, corporate actions, commissions, and slippage.
...
- [mx-risk-guard](https://github.com/27dream/mx-risk-guard) - `Python` - Broker-agnostic A-share trading risk control engine with concentration limits, daily loss circuit breakers, drawdown stop-loss, and blacklist management; pure Python with zero LLM dependency.
...
- [mcp-eastmoney](https://github.com/27dream/mcp-eastmoney) - `Python` - MCP protocol server providing real-time A-share market data via East Money (东方财富) API; no API key required, works with Claude/Cursor/Codex and any MCP client, covering tick-level quotes, K-line, capital flow, and sector data.
