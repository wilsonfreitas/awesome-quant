...
- [History of Market](https://historyofmarket.com) - `Data` - A century of U.S. stock-market history (S&P 500 since 1928, Nasdaq, Dow, SOX, sector ETFs, Magnificent 7) as 110 charts, each backed by a CORS-open JSON endpoint under `/api/`, refreshed after every U.S. close. No API key; OpenAPI 3.1 spec, `llms.txt` and an agent site card at `/api/profile.json`. Data CC BY 4.0.
