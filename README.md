...
- [APITube](https://apitube.io) - `REST` `MCP` - News API returning articles as JSON with article-level and entity-level sentiment scores, resolved named entities, IPTC categories and industry tags, filterable by entity ID, language, source country and publication date range over an archive reaching back to 2020. Permanent free tier: 100 requests a day, 10 articles per request, no card; on free the article body is truncated and the hosted MCP server is not included, paid plans are $29, $99 and $199 a month. [GitHub](https://github.com/apitube/news-api-node)
