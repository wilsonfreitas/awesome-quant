...
- [Shingou](https://shingou.io) - `TypeScript` `REST API` `MCP` - Hourly news-sentiment scores and typed market events for 30 crypto pairs, with point-in-time buckets and every hourly bucket's hash committed to a public append-only log at publish time so history can be verified as never rewritten before you backtest against it. [GitHub](https://github.com/auriontech/shingou-integrations)
