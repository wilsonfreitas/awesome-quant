...
- [CoinNudge](https://coinnudge.site/data) - `REST` `CSV` - Crypto market-research service with a permanent free-account tier for published Research articles and current JSON/CSV datasets, including one API key and a shared limit of 60 requests per minute; historical bulk exports and the versioned Market Events dataset are paid or separately scoped. [Samples](https://coinnudge.site/data/samples) | [API docs](https://coinnudge.site/data/api-docs)
