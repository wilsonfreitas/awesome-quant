...
## Trading & Backtesting
- [cl-lp-rotation-scanner](https://github.com/donnywin85/cl-lp-rotation-scanner) - `Python` - Scores hedgeable concentrated-liquidity pools by a spike-resistant median fee floor with closed-form CL impermanent-loss math over each pool trailing price path, then backtests pool rotation against the no-lookahead ex-ante static benchmark instead of the hindsight-best pool. Keyless, standard library only.
...
- [perp-funding-collector](https://github.com/donnywin85/perp-funding-collector) - `Python` - Append-only funding-rate collector for five DEX perp venues (Hyperliquid, dYdX v4, Aster, Paradex, Lighter), normalising every venue rate to a per-hour figure and flagging rows below a $2M open-interest floor at the sensor so undepthed headline APRs never reach a consumer. Keyless, read-only, standard library only.
