...
- [invinoveritas /review](https://api.babyblueviper.com) - `Python` `MCP` - Pre-execution governance gate for AI trading agents — capital-scale-aware advisory verdict (approve / approve_with_concerns / reject) before an order is placed. MCP server + REST + x402 (USDC) + Lightning pay-per-call. Live Hyperliquid bot dogfood; verdicts signed + recomputable via public ledger. [GitHub](https://github.com/trustless-ai/agent-contracts-examples)
