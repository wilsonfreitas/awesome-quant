...
## Trading & Backtesting
- [AgentQuant](https://github.com/OnePunchMonk/AgentQuant) - `Python` - Autonomous trading research agent running a ReAct loop (analyze, hypothesize, backtest, reflect, store, improve) with cross-session SQLite memory and walk-forward validation; evolves its own harness parameters via genetic algorithms and differential evolution, and tracks the accuracy of its own falsifiable Sharpe predictions.
...
- [itch-book](https://pypi.org/project/itch-book/) - `Python` - NASDAQ TotalView-ITCH 5.0 day files to Parquet: BBO, trades, order-by-order messages with the resting side already resolved, and MBP depth; downloads and verifies the free emi.nasdaq.com samples. [GitHub](https://github.com/groovg/itch-book)
